...
#define doErase p.x>47 && p.x<234 && p.y>218 && p.y<268 && inApp == 1 // Erase button, confirmed. This will start the obliteration.
#define cancelErase p.x>248 && p.x<435 && p.y>218 && p.y<268 && inApp == 1// Cancel button, it will revert back to Core Storage Media
...
// Misc objects
#define appExitButton p.x>10 && p.x<34 && p.y>292 && p.y<316 && inApp == 1// Cancel button, it will revert back to Core Storage Media
